-- Separate profile completion from explicit membership application submission.
...
create unique index if not exists membership_applications_user_org_unique_idx
  on public.membership_applications (user_id, organization_id);
...
drop policy if exists "membership_applications_update_member_managers" on public.membership_applications;
create policy "membership_applications_update_member_managers"
on public.membership_applications
for update using (
...
begin
  ...
  missing_fields := array_remove(array[
    ...
    case when nullif(btrim(coalesce(profile_record.club_affiliation, '')), '') is null then 'clubAffiliation' end
  ], null);
