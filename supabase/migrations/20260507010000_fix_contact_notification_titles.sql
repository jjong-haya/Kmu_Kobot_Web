-- Contact request notification titles were originally persisted by RPCs.
-- Replacing the RPCs here is required because editing an old migration does
-- not update already-applied database functions in Supabase.
...
set title = case
  when type = 'contact_request_created' then '연락 요청이 도착했습니다'
  when type = 'contact_request_decided' and metadata ->> 'decision' = 'accepted' then '연락 요청이 수락되었습니다'
  when type = 'contact_request_decided' and metadata ->> 'decision' = 'rejected' then '연락 요청이 거절되었습니다'
  else title
end
where related_entity_table = 'contact_requests'
  and type in ('contact_request_created', 'contact_request_decided');
