...
| Q-AUTH-002 | P0 | Member Registry | What is the final canonical member status set? | TypeScript includes `project_only` and `withdrawn`; older DB status did not. Policy also mentions inactive/suspended terms. | Use `pending`, `active`, `suspended`, `rejected`, `alumni`, `project_only`, `withdrawn`. | active | Verify DB constraints and migration history. |
...
| Q-AUTH-004 | P0 | Identity And Access > Login ID | Should `resolve_login_email` allow only `active` accounts, or allow pending/suspended/alumni login and block inside the app? | Current SQL excludes only `rejected`; pending/suspended/alumni could still sign in by ID. | Allow sign-in only for `active` unless product explicitly wants pending ID login. | active | Decide and update RPC if needed. |
...
| Q-PROFILE-001 | P0 | Member Registry > Profile Identity | Is public attribution default `anonymous` or `nickname`? | User decisions said public page default should be anonymous, but some code defaults differ. | Public pages default to `anonymous`; internal pages show real name/nickname by context. | active | Align DB/default UI/docs. |
...
| Q-AUTHZ-001 | P0 | Authority And Capability | Should `maintainer` be removed, renamed to `project_operator`, or migrated with restricted capability? | Current helper may treat maintainer like a lead. | Use `project_operator` as scoped helper; do not give lead-transfer/private-global powers. | active | Add migration/task. |
...
| Q-VOTE-001 | P0 | Voting And Governance > Anonymous Vote | Does anonymous vote mean normal operator UI anonymity, president-only audit access, or database-level unlinkability? | User expectation and DB structure can diverge. | Normal UI shows aggregate only; president audit access must be explicitly disclosed if retained. | active | Decide anonymity level per vote type. |
...
| Q-AUDIT-001 | P0 | Audit Notification And Retention | Can normal active members insert audit logs directly? | Current RLS allowed active member insert; this permits forged audit history. | Only command RPCs/triggers/service role insert audit logs. | active | Add migration to restrict direct insert. |
