...
| Q-VOTE-002 | 투표 자격을 투표 시작 시점에 고정할 것인가 | 투표 중 멤버/팀 상태가 바뀌면 자격이 흔들릴 수 있음 |

## 7. 투표 익명성 확정

익명 투표는 다음 수준으로 확정했습니다.

- 일반 부원, 프로젝트 팀장, 공식 팀장, 부회장, 일반 운영 화면에서는 집계 결과만 볼 수 있습니다.
- 회장만 감사 목적으로 개별 투표 기록을 확인할 수 있습니다.
- 투표 제출 전 화면에는 이 사실을 반드시 고지해야 합니다.
- 따라서 구현 시에는 `집계 결과 read model`과 `회장 감사용 개별 ballot read model`을 분리해야 합니다.

## 8. 추천 기본값 일괄 적용

남은 제품 결정은 사용자의 승인에 따라 Codex 추천 기본값으로 확정했습니다.

- `project_only` 사용자는 프로필, 알림, 초대/참여한 프로젝트 공간만 접근합니다.
- 닉네임 7일 제한과 숨김 이력 정책은 DB/RPC/RLS에서 강제합니다.
- 프로젝트 가입 승인/거절, 초대 코드 사용, 연락 요청, 투표 제출 같은 상태 전이는 command RPC로 처리합니다.
- private GitHub README snapshot은 최신본 1개만 기본 보관합니다.
- 공개 공지와 내부 공지는 별도 read model로 분리합니다.
- production UI에서는 `mock`, `DB 연결 전` 같은 개발 상태 문구를 제거합니다.

## 9. 이번 추가 구현

가입 정보 작성과 가입 요청 제출을 분리했습니다.

- 새 테이블: `membership_applications`
- 새 RPC: `submit_current_membership_application()`
- 프론트 연결: pending 사용자가 가입 정보를 저장하면 명시적 가입 요청 row가 생성됩니다.
- 감사 로그에는 raw 개인정보 대신 필드 존재 여부만 기록합니다.
